...
class TPUModelRunner(KVConnectorModelRunnerMixin, LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        # Connector transfer preparation may run on a background thread.
        # Serialize JAX dispatches that consume or replace KV cache buffers so
        # a gather cannot race a model forward using donated buffers.
        self.kv_cache_lock = threading.RLock()
